...
The `audience-refiner` skill drives the Primer audience API-key endpoints
...
`reference/api-contract.md` is the authoritative, generated reference for each
endpoint's mechanical shape; the CLI ↔ endpoint map in `SKILL.md` is the
checklist tying each verb to its endpoint. If a call's request or response shape
ever disagrees with the contract, trust the contract and report the discrepancy.
